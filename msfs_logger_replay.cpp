...
//********************   VERSION HISTORY          ********************************
//********************************************************************************
// 2.31  * bugfix for non-English Flight Simulator X Files
// 2.30  * uses 'documents\Flight Simulator X Files\sim_connect_unverified_logs
// 2.29  * looking again at Unicode load of flightplans for plan-G
// 2.28  * store 'free flight' tracklogs in Modules\sim_logger\free_flight_logs
// 2.27  * update IGC filename date/time on 'restart log'
// 2.26  * enable_autosave ini setting
// 2.24/5* prod release - 2.25 bugfix zip with language subfolder support
// 2.23  * trying gear up/down (commented out for now)
//       * enable/disable replay menu
//       * checksum cumulusx.exe
// 2.21  * pitch fixed
...
const INT32 IGC_MIN_RECORDS = 20*60; // don't auto-save an IGC file if it is < 20 mins
...
wchar_t LANG_SUB_PATH[] = L"Modules\\sim_logger\\language\\lang_";
// default folder for free flight tracklog files
wchar_t LOG_SUB_PATH[] = L"sim_logger_unverified_logs";
wchar_t FSXBASE[MAXBUF]; // pathname to FSX base folder (current dir when sim_logger is loaded)
wchar_t FSXFILES[MAXBUF]; // pathname to 'Flight Simulator X Files' folder
...
char lang_enable_replay[MAXBUF] = "ENABLE tracklog replay (currently disabled)";
char lang_disable_replay[MAXBUF] = "DISABLE tracklog replay (currently enabled)";
...
char *LANG_UNVERIFIED = "unverified";
...
char chksum_cx[CHKSUM_CHARS+1] = "000000"; // checksum for CumulusX.exe
...
wchar_t flt_start_time[MAXBUF] = L"";
...
// events for the FSX menus put up by logger under add-ons
	...
	EVENT_MENU_TEXT,
    EVENT_MENU_ENABLE_REPLAY, // main menu item
    EVENT_MENU_DISABLE_REPLAY, // main menu item
// ai control events
...
void load_ini() {
	...
	wchar_t buf[MAXBUF];
	DWORD length;
    float float_buf; // temp var just to hold the parsed 'float' value from .ini file
	...
	length = GetPrivateProfileString(INI_APP_NAME,
										...
										MAXBUF,
										ini_path);
	float_buf = 30; // default v_zero = 30 m/s
	swscanf_s(buf,L"%f",&float_buf);
	...
	length = GetPrivateProfileString(INI_APP_NAME,
										...
										MAXBUF,
										ini_path);
	if (_wcsicmp(buf, L"")==0) ini_enable_autosave = true;
	else if (_wcsicmp(buf, L"false")==0) ini_enable_autosave = false;
	else if (_wcsicmp(buf, L"0")==0) ini_enable_autosave = false;
	else ini_enable_autosave = true;
	if (debug) printf("INI: enable_autosave = %s\n", (ini_enable_autosave) ? "true":"false");
}

// write or update a key / value pair to the ini file
void ini_write(wchar_t *key, wchar_t *value) {
    wchar_t ini_path[MAXBUF];
	// build full sim_logger.ini path
	wcscpy_s(ini_path, MAXBUF, FSXBASE);
	wcscat_s(ini_path, MAXBUF, INI_SUB_PATH);
	// log_directory
	BOOL success = WritePrivateProfileString(INI_APP_NAME,
										key,
										value,
										ini_path);
}
...
void load_lang() {
    ...
    get_lang_string("enable_replay",lang_enable_replay,
                    "ENABLE tracklog replay (currently disabled)", ini_path);
    get_lang_string("disable_replay",lang_disable_replay,
                    "DISABLE tracklog replay (currently enabled)", ini_path);
}
...
CHKSUM_RESULT chksum_cumulusx_exe() {
    wchar_t CX_SUB_PATH[] = L"\\Modules\\CumulusX!\\CumulusX.exe";
    wchar_t cx_path[MAXBUF];
    char buf[MAXBUF];
	// build full cumulusx.exe path
	wcscpy_s(cx_path, MAXBUF, FSXBASE);
	wcscat_s(cx_path, MAXBUF, CX_SUB_PATH);
    clean_string(buf, cx_path);
    if (debug) printf("Finding checksum for %s\n", buf);
    return chksum_binary_file(chksum_cx, buf);
}
...
CHKSUM_RESULT chksum_chksum(char chksum[CHKSUM_CHARS+1]) {
	...
	chksum_string(&chk_data,chksum_cx);
...
CHKSUM_RESULT pln_to_c(char *filepath) {
	FILE *f;
	errno_t err;
	const char LLA_FORMAT[] = "%c %d %*s %d' %f\", %c %d %*s %d' %f";
    ...
    while (fgetws(in_buf, MAXBUF, f)!=NULL) {
        ...
        if (ptr_value!=NULL) {
            ptr_end = strchr(ptr_value+1, '<');
            if (ptr_end!=NULL) {
                sscanf_s(ptr_value+16, "%c %d %*s %d' %f\", %c %d %*s %d' %f",
                    &lat_NS,1, &lat_degs, &lat_mins, &lat_secs,
                    &long_NS,1, &long_degs, &long_mins, &long_secs);
                ...
                // debug                C  DD    MM   MMM   N   DDD   MM    MMM  E
                //if (debug) printf("DestinationLLA \"C %02.2d %02.2d %03.0f %c %03.3d %02.2d %03.0f %c\"\n",
                //    lat_degs, lat_mins, lat_secs, lat_NS, 
                //    long_degs, long_mins, long_secs, long_NS );
            }
            continue;
        ...
        if (ptr_value!=NULL) {
            ptr_end = strchr(ptr_value+1, '<');
            if (ptr_end!=NULL) {
                sscanf_s(ptr_value+15, "%c %d %*s %d' %f\", %c %d %*s %d' %f",
                    &lat_NS,1, &lat_degs, &lat_mins, &lat_secs,
                    &long_NS,1, &long_degs, &long_mins, &long_secs);
                ...
                sprintf_s(s,MAXBUF,"C%02.2d%02.2d%03.0f%c%03.3d%02.2d%03.0f%c",
                    lat_degs, lat_mins, lat_secs, lat_NS, 
                    long_degs, long_mins, long_secs, long_NS );
                // roll back one waypoint if this is the same as previous
                if (c_wp_count>1 && strncmp(s,c[c_wp_count],18)==0) c_wp_count--;
                else cpy(c[c_wp_count+1],MAXBUF,s,18);
                //debug                C  DD    MM   MMM   N   DDD   MM    MMM  E
...
void igc_get_flight_start_time() {
	__time64_t ltime;
	struct tm today;
    _time64(&ltime);
    _localtime64_s( &today, &ltime );
	wcsftime(flt_start_time, MAXBUF, L"%Y-%m-%d_%H%M", &today );
}

void igc_restart() {
	char restart_text[MAXBUF];
    char buf[MAXBUF];

    igc_get_flight_start_time();
    igc_reset_log();
	
	clean_string(buf, flt_start_time);
	sprintf_s(restart_text, 
			sizeof(restart_text), 
			"%s %s", 
			lang_reset, 
			buf);
...
void igc_write_file(wchar_t *reason) {
	...
	// path_to_name(wx_name, wx_pathname); done on FLT load
	...
	// make wflight_filename[] = the FLT filename without the .FLT
	// i.e. "c:\abc\def\my flight.FLT" -> "my flight"
    if (free_flight()) { // free flight
        wcscpy_s(wflight_filename, L"free_flight");
    } else {
	...
	if (wcslen(ini_log_directory)==0) { // no entry for log folder in ini file
        if (free_flight()) {
	        wcscpy_s(fn, MAXBUF, FSXFILES);
	        wcscat_s(fn, MAXBUF, LOG_SUB_PATH);
        } else {        
	...
	if (!free_flight()) wcscat_s(fn, MAXBUF, wflight_filename);
	...
	wcscat_s(fn, L"_");
    if (wcslen(flt_start_time)==0) igc_get_flight_start_time();
    wcscat_s(fn, flt_start_time);
	if (wcslen(reason)>1) {
		wcscat_s(fn, MAXBUF, L"(");
	...
	if( (err = _wfopen_s(&f, fn, L"w")) != 0 ) {
        ...
        clean_string(s2, flt_start_time);
		...
		sprintf_s(s,MAXBUF,		   "L FSX CumulusX.exe checksum   %s\n", chksum_cx);
		chksum_string(&chk_data, s); fprintf(f, s);
...
void flush_igc(wchar_t *reason) {
    if (ini_enable_autosave && !igc_saved && igc_record_count>IGC_MIN_RECORDS) {
...
struct AIInfo {
    ...
    bool slew_on; // slew status (used for gear animations)
	//debug
...
double desired_pitch(double alt_delta, double dist, double time) {
    ...
    // C & X are a couple of co-efficients derived from pitch min & max
...
void ai_set_slew(int ai_index, bool on) {
	HRESULT hr;
    if (debug) printf("SLEW %s to %s\n",
        (ai_info[ai_index].slew_on) ? "ON" : "OFF",
        (on) ? "ON" : "OFF");
    ai_info[ai_index].slew_on = on;
    if (on)
	    hr = SimConnect_TransmitClientEvent(hSimConnect,
										ai_info[ai_index].id,
										EVENT_SLEW_ON,
...
void ai_gear(int ai_index, int current_target, AIStruct pos) {
	...
	if (!ai_info[ai_index].gear_up && pos.altitude_agl > GEAR_UP_HEIGHT) {
        ...
        // set slew OFF - will reset to ON on next update
        ai_set_slew(ai_index, false);
		hr = SimConnect_TransmitClientEvent(hSimConnect,
										ai_info[ai_index].id,
										EVENT_GEAR_UP,
...
void update_ai(int ai_index, AIStruct pos) {
    ...
    if (found) { // i.e. we have also found the predict point
        ...
        if (false && debug) printf(",target:,%d,%2.5f,%3.5f,%5.0f,%+2.5f,%+2.5f,%+2.5f\n",
                            ...
                            r[i].bank,
                            r[i].heading );
        // set slew back to ON if needed
        if (!ai_info[ai_index].slew_on) ai_set_slew(ai_index, true);
        ...
        hr = SimConnect_TransmitClientEvent(hSimConnect,
						    ...
						    SIMCONNECT_GROUP_PRIORITY_HIGHEST,
						    SIMCONNECT_EVENT_FLAG_GROUPID_IS_PRIORITY);
		// send gear up/down as necessary - debug commented out for now
...
// calculate appropriate pitch/bank/heading values for replaypoint[i]
...
int load_igc_file(int ai_index, wchar_t path[MAXBUF]) {
	...
	if(_waccess_s(path, 0) != 0) {
		...
		ai_info[ai_index].slew_on = false;
		return 0;
	}
}
...
void free_flight_load() {
    wcscpy_s(flt_directory, MAXBUF, L"");
    strcpy_s(chksum_flt, CHKSUM_CHARS+1, "000000");
    strcpy_s(flt_name, MAXBUF, LANG_UNVERIFIED);
    strcpy_s(flt_pathname, "");
}
...
void process_flt_load_msg(char *flt_filepath) {
    ...
    // now checksum the cumulusx.exe file
    chksum_cumulusx_exe();
...
char menu_tracklog_cx_checksum[MAXBUF];
...
void delete_enable_replay_menu(EVENT_ID e) {
    HRESULT hr = SimConnect_MenuDeleteSubItem(hSimConnect, EVENT_LOGGER_MENU, e);
}

// add the appropriate replay enable or disable sub-menu
void add_enable_replay_menu(EVENT_ID e) {
    if (e == EVENT_MENU_DISABLE_REPLAY)
	    HRESULT hr = SimConnect_MenuAddSubItem(hSimConnect, 
                                    EVENT_LOGGER_MENU, 
                                    lang_disable_replay, 
                                    EVENT_MENU_DISABLE_REPLAY, 0);
    else
	    HRESULT hr = SimConnect_MenuAddSubItem(hSimConnect, 
                                    EVENT_LOGGER_MENU, 
                                    lang_enable_replay, 
                                    EVENT_MENU_ENABLE_REPLAY, 0);
}
...
void create_addon_menu() {
	...
	hr = SimConnect_MapClientEventToSimEvent(hSimConnect, EVENT_MENU_ENABLE_REPLAY);
	hr = SimConnect_MapClientEventToSimEvent(hSimConnect, EVENT_MENU_DISABLE_REPLAY);
    ...
    // now add the appropriate enable replay sub-menu
    if (ini_enable_replay) add_enable_replay_menu(EVENT_MENU_DISABLE_REPLAY);
    else add_enable_replay_menu(EVENT_MENU_ENABLE_REPLAY);
...
void menu_enable_replay(EVENT_ID e) {
    switch (e) {
        case EVENT_MENU_DISABLE_REPLAY:
            // replay currently enabled
            // disable replay in ini file
            if (debug) printf("ini_enable_replay: %s", (ini_enable_replay)?"TRUE":"FALSE");
            ini_write(L"enable_replay", L"false");
            delete_enable_replay_menu(EVENT_MENU_DISABLE_REPLAY);
            add_enable_replay_menu(EVENT_MENU_ENABLE_REPLAY);
            ini_enable_replay = false;
            if (debug) printf(" ini_enable_replay: %s\n", (ini_enable_replay)?"TRUE":"FALSE");
            reset_ai();
            break;

        case EVENT_MENU_ENABLE_REPLAY:
            // replay currently disabled
            // enable replay in ini file
            if (debug) printf("ini_enable_replay: %s", (ini_enable_replay)?"TRUE":"FALSE");
            ini_write(L"enable_replay", L"true");
            delete_enable_replay_menu(EVENT_MENU_ENABLE_REPLAY);
            add_enable_replay_menu(EVENT_MENU_DISABLE_REPLAY);
            ini_enable_replay = true;
            if (debug) printf(" ini_enable_replay: %s\n", (ini_enable_replay)?"TRUE":"FALSE");
            break;
    }
}
...
void menu_tracklog_init() {
    ...
    strcpy_s(menu_tracklog_cx_checksum,"(not in tracklog)");
...
void menu_tracklog(char *filename) {
	...
	while (fgets(line_buf, MAXBUF, f)!=NULL) {
        ...
        if (igc_read(line_buf, "L FSX CumulusX.exe checksum   ",
            menu_tracklog_cx_checksum)) continue;
...
void menu_tracklog_detail() {
    ...
    strcpy_s(pc, MAXBUF, lang_weather);
	pc += strlen(lang_weather);
    sprintf_s(s, MAXBUF, "CumulusX! checksum %s, %s", menu_tracklog_cx_checksum, menu_tracklog_cx_status);
	strcpy_s(pc, MAXBUF, s);
	pc += strlen(s)+1;
    ...
    switch(pData->dwID)
    {
        case SIMCONNECT_RECV_ID_EVENT:
        {
            ...
            switch(evt->uEventID)
            {
				...
				case EVENT_MENU_ENABLE_REPLAY:
					if (debug) printf(" [EVENT_MENU_ENABLE_REPLAY] ");
					menu_enable_replay(EVENT_MENU_ENABLE_REPLAY);
                    break;
					
				case EVENT_MENU_DISABLE_REPLAY:
					if (debug) printf(" [EVENT_MENU_DISABLE_REPLAY] ");
					menu_enable_replay(EVENT_MENU_DISABLE_REPLAY);
                    break;
        ...
        case SIMCONNECT_RECV_ID_EVENT_WEATHER_MODE:
        {
            ...
            switch(evt->uEventID)
            {
				case EVENT_WEATHER: // User has changed weather
					...
					wx_code = 0;
                    strcpy_s(chksum_wx, CHKSUM_CHARS+1, "000000");
                    strcpy_s(wx_name, MAXBUF, "free flight");
					break;
...
void connectToSim()
{
    ...
    if (SUCCEEDED(SimConnect_Open(&hSimConnect, sim_connect_string, NULL, 0, 0, 0)))
    {
		...
		hr = SimConnect_MapClientEventToSimEvent(hSimConnect, EVENT_SLEW_OFF, "SLEW_OFF");
...
int main(int argc, char* argv[])
{
	...
	// set up user documents path
    wchar_t my_docs[MAXBUF];
	HRESULT hr = SHGetFolderPath(NULL, CSIDL_PERSONAL, NULL,
                             SHGFP_TYPE_CURRENT, my_docs);
	wcscat_s(my_docs, L"\\");
	if (debug) wprintf(L"User documents=%s\n", my_docs);

	// set up FSXFILES path
	wcscpy_s(FSXFILES, my_docs); // start from my docs folder
	...
	{ 
		...
		if (LoadStringW(hInstLang, 36864, &fsx_files[0], 128)!=0) {
			if (debug) wprintf(L"FSX files folder name (from language.dll)=%s\n",fsx_files);
	...
	wcscat_s(FSXFILES,L"\\");
	if (debug) wprintf(L"FSXFILES path=%s\n",FSXFILES);
